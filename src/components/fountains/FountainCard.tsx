...
// Raw rating strings displayed directly; numeric/grade conversion removed.
  ...
  return (
    <Link href={`/fountains/${fountain.id}`}>
      <div className="bg-gray-900 p-4 rounded-2xl shadow hover:shadow-lg transition">
        ...
        {fountain.flavorRating && (
          <div className="flex items-center gap-1 mt-1">
            <Droplets className="w-4 h-4 text-blue-500" />
            <span className="font-medium whitespace-nowrap text-xs md:text-sm">
              {fountain.flavorRating}
            </span>
          </div>
        )}
